...
For this project, keep test changes aligned with the established suite:

1. Shared pytest fixtures belong in `tests/conftest.py`.
2. Reusable test-data builders belong in `tests/factories.py`.
3. Keep tests organized by domain; do not leave duplicate old/new test trees.
4. Do not duplicate scenarios or setup when a clear shared helper or parametrized variant is appropriate.
5. Preserve regression, security/RBAC, migration, cross-record isolation, and domain-invariant tests.
6. Mark genuinely expensive tests `slow` and migration-specific tests `migration`; still run required marked coverage at checkpoints.
7. Ordinary test users may use valid low-cost password hashes, but password/security-path tests must use the real production path.
8. Delete a test only when its coverage is demonstrably duplicate, obsolete, or superseded by a stronger current test.
9. Do not introduce shared database state, transactional shortcuts, or parallel execution until isolation is proven.
10. After test reorganization, verify collection before the full run so old and new paths are not both collected.
